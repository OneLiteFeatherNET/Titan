...
class ArchitectureTest {
    ...
    /**
     * True for a class named {@code *Module}, a class named {@code *Config}, a class nested
     * (directly or transitively) inside a class named {@code *Config}, or a class Avaje Inject's
     * annotation processor generated (e.g. {@code ElytraModule$DI}) - generated wiring code, not
     * hand-authored feature surface, so decision 10.3's "only *Module and *Config are public"
     * intent does not apply to it; see
     * {@code openspec/changes/avaje-dependency-injection/design.md}. The generated-class check
     * requires both the {@code $DI} simple-name suffix Avaje Inject's generator uses and the
     ...
     * annotation.
     */
    private static final DescribedPredicate<JavaClass> PUBLIC_FEATURE_API = DescribedPredicate.describe("named '*Module' or '*Config', nested in a class named '*Config', or generated by Avaje Inject", javaClass -> isModuleOrConfigOrNestedInConfig(javaClass) || isAvajeGeneratedType(javaClass));
    ...
    private static boolean isAvajeGeneratedType(JavaClass javaClass) {
        // getName() (the fully qualified binary name), not getSimpleName(): ArchUnit parses a
        // "$"-containing binary name as if it always denoted a nested class, so for Avaje
        // Inject's generated top-level "FooModule$DI" class, getSimpleName() returns just "DI",
        // not "FooModule$DI" - matching on it here would be far too broad (any class simply
        // named "DI" anywhere). getName() is the full binary name and is not subject to that
        // parsing.
        return javaClass.getName().endsWith("$DI") && javaClass.isAnnotatedWith(Generated.class);
    }
