 ...
 * {@link net.onelitefeather.titan.common.config.ConfigException} naming the broken file and the
 * parser's position (see the {@code lobby-module-config} spec scenario "Syntaktisch kaputte
 * Datei").
 *
 ...
 * Independent/Repeatable (F.I.R.S.T.) - see {@link
 * ConfigurationPrecedenceTest#brokenApplicationYamlAbortsCleanlyNamingFileAndPosition} for that
 * coverage. What is tested here, hermetically, is the message-parsing logic itself against a
 * fabricated exception shaped exactly like the one {@code avaje-config}'s {@code InitialLoader}
 * throws (confirmed by decompiling {@code avaje-config:5.2}): {@code new
 * IllegalStateException("Error loading properties - " + resourceName, cause)}.
 */
