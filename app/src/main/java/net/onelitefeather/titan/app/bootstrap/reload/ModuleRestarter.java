 ...
 *
 * <p>The production adapter ({@link ModuleRestarterAdapter}) delegates to
 ...
 * javadoc for why {@link ConfigChangeHandler} does not depend on the {@code module} package
 * directly.
 ...
 * the {@code module} package, is the only ordering seam {@link ConfigChangeHandler} knows about.
 */
public interface ModuleRestarter {
     ...
     * alphabetical order {@link ModuleKeys#affectedModuleIds} happens to iterate in. See
     * {@code openspec/changes/config-reload-feature-flags/design.md}, decision 3: restarts follow
     * registration order, so {@link ConfigChangeHandler} restarts affected modules in the order
     * this
     * method returns them, skipping any affected id this restarter does not know - a changed key
     * whose prefix never named a module (e.g. a typo, or a section such as {@code spawn} with no
     * matching module) is deliberately never passed to {@link #restart(String)}.
     *
