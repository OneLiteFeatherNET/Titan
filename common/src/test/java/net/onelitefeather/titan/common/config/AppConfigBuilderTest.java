...
class AppConfigBuilderTest {
	...
	void testBuilderCreatesAppConfigWithCorrectValues() {
		...
		// Act
		AppConfig appConfig = AppConfig.builder().tickleDuration(tickleDuration).sitOffset(sitOffset)
				.allowedSitBlocks(allowedSitBlocks).simulationDistance(simulationDistance)
				.fireworkBoostSlot(fireworkBoostSlot).elytraBoostMultiplier(elytraBoostMultiplier)
				.minHeightBeforeTeleport(minHeightBeforeTeleport).maxHeightBeforeTeleport(maxHeightBeforeTeleport)
				.build();
	...
	void testBuilderWithExistingAppConfig() {
		// Arrange
		AppConfig originalConfig = AppConfig.builder().tickleDuration(1000L).sitOffset(new Vec(0, 1.5, 0))
	...
	void testAddAndRemoveAllowedSitBlocks() {
		...
		// Act
		AppConfig appConfig = AppConfig.builder().allowedSitBlocks(initialBlocks).addAllowedSitBlock(stoneStairsKey)
		...
		// Act - Remove a block
		AppConfig updatedConfig = AppConfig.builder(appConfig).removeAllowedSitBlock(stoneStairsKey)
