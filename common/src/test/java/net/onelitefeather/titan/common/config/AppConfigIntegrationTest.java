...
class AppConfigIntegrationTest {
	...
	void testFullConfigurationLifecycle() {
		...
		AppConfig initialConfig = AppConfig.builder().tickleDuration(1000L).sitOffset(new Vec(0, 1.5, 0))
				.allowedSitBlocks(new ArrayList<>()).simulationDistance(10).fireworkBoostSlot(8)
				.elytraBoostMultiplier(1.5).minHeightBeforeTeleport(-64).maxHeightBeforeTeleport(320).build();
		...
		AppConfig configWithBlocks = AppConfig.builder(initialConfig).addAllowedSitBlock(Material.OAK_STAIRS)
				.addAllowedSitBlock(Material.STONE_STAIRS).build();
		...
		AppConfig configWithOneBlock = AppConfig.builder(configWithBlocks).removeAllowedSitBlock(Material.STONE_STAIRS)
				.build();
	...
	void testConfigurationWithMultipleChanges() {
		// Create initial configuration
		AppConfig initialConfig = AppConfig.builder().tickleDuration(1000L).sitOffset(new Vec(0, 1.5, 0))
				.allowedSitBlocks(List.of(Key.key("minecraft:oak_stairs"))).simulationDistance(10).fireworkBoostSlot(8)
				.elytraBoostMultiplier(1.5).minHeightBeforeTeleport(-64).maxHeightBeforeTeleport(320).build();

		// Make multiple changes
		AppConfig modifiedConfig = AppConfig.builder(initialConfig).tickleDuration(2000L).sitOffset(new Vec(0, 2.0, 0))
				.simulationDistance(15).fireworkBoostSlot(9).elytraBoostMultiplier(2.0).minHeightBeforeTeleport(-128)
				.maxHeightBeforeTeleport(384).build();
